...
const LegacyRedirect = () => {
  ...
  if (isValidEdition(param)) {
    if (slug) {
      return <Navigate to={`/app/${param}/items/${slug}`} replace />;
    }
    return <Navigate to={`/app/${param}/items`} replace />;
  }

  // If param is not an edition, assume it's a slug and use default edition
  return <Navigate to={`/app/${DEFAULT_EDITION}/items/${param}`} replace />;
};
...
const DMRoutes = (props: ResolverProps) => {
  return (
    <Routes>
      ...
      {/* Legacy redirects for old URL structure */}
      <Route path="/app/items/:param/:slug" element={<LegacyRedirect />} />
      <Route path="/app/items/:param" element={<LegacyRedirect />} />
      <Route
        path="/app/items"
