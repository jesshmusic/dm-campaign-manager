...
class NameGen
  ...
  class << self
    ...
    def random_monster_name
      options = {
        name1: @monster_name1.clone.map(&:clone),
        name2: @monster_name2.clone.map(&:clone),
        name3: @monster_name3.clone.map(&:clone),
        name4: @monster_name4.clone.map(&:clone),
        name5: @monster_name5.clone.map(&:clone),
        name6: @monster_name6.clone.map(&:clone),
        name7: @monster_name7.clone.map(&:clone),
      }
      monster_name = @monster_patterns.sample
      monster_name.gsub(/<([\w\W]*?)>/) { |match|
        match = match.gsub(/<|>/, '')
        options[match.to_sym].sample
      }.titleize
    end
