...
RSpec.describe VehicleItem, type: :model do
  context "with the same name" do
    ...
    it "generates unique slugs" do
      @item = VehicleItem.create!(name: 'Test Unique Item', weight: 10)
      @item1 = VehicleItem.create!(name: 'Test Unique Item', weight: 10)
      @user_item = VehicleItem.create!(name: 'Test Unique Item', weight: 10, user: dungeon_master)
    ...
    it "maintains same slug on update with no name change" do
      initial_count = VehicleItem.count
      @item = VehicleItem.create!(name: 'Test Unique Item', weight: 10)
      @item1 = VehicleItem.create!(name: 'Test Unique Item', weight: 10)
      @user_item = VehicleItem.create!(name: 'Test Unique Item', weight: 10, user: dungeon_master)
      ...
      expect(VehicleItem.all.count).to eq(initial_count + 3)
      @item.reload
